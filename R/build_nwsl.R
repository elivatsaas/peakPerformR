#' Build NWSL Player Data
#'
#' @description
#' Builds a dataset of NWSL (National Women's Soccer League) players with performance data from
...
#' @param start_year Integer. First year to fetch data for when rebuilding. Default is 2016.
#' @param end_year Integer. Last year to fetch data for when rebuilding. Default is current year.
#' @param ... Additional arguments passed to internal functions.
#'
#' @return A data frame containing NWSL player data if output_path is NULL.
#'         Otherwise, writes to the specified path and returns that path invisibly.
#'
#' @importFrom itscalledsoccer AmericanSoccerAnalysis
#' @importFrom magrittr %>%
#' @importFrom dplyr select inner_join filter mutate group_by summarize ungroup rename bind_rows sym case_when row_number n across everything
...
#' nwsl_data <- build_nwsl()
#'
#' # Rebuild data from APIs (takes time)
#' nwsl_data_fresh <- build_nwsl(use_cached = FALSE)
#'
#' # Get data for specific years
#' nwsl_data_recent <- build_nwsl(use_cached = FALSE, start_year = 2020, end_year = 2023)
#'
#' # Save to specific location
#' build_nwsl(output_path = "path/to/save/directory")
#' }
#'
#' @export
build_nwsl <- function(output_path = NULL, use_cached = TRUE, start_year = 2016,
  ...
  if(use_cached) {
    nwsl_data <- tryCatch({
      ...
      nwsl_tidy <- NULL
      data("nwsl_tidy", envir = environment())
      nwsl_tidy
    }, error = function(e) {
      # If loading package data fails, try to load from CSV
      csv_path <- system.file("extdata", "nwsl_final.csv", package = "yourpackage")
      if (csv_path == "") {
        warning("Could not find cached NWSL data. Attempting to rebuild from APIs.")
        return(NULL)
      }
      tryCatch({
        read.csv(csv_path)
    ...
    if (!is.null(nwsl_data)) {
      if (!is.null(output_path)) {
        # Create directory if it doesn't exist
        if (!dir.exists(output_path)) {
          dir.create(output_path, recursive = TRUE)
        }
        write.csv(nwsl_data, file.path(output_path, "nwsl_tidy.csv"), row.names = FALSE)
        message("NWSL data saved to ", file.path(output_path, "nwsl_tidy.csv"))
        return(invisible(file.path(output_path, "nwsl_tidy.csv")))
      } else {
        return(nwsl_data)
      }
    }
  ...
  message("Building NWSL data from APIs. This may take some time...")
  ...
  # Ensure consistent position designation for goalies
  summary_goalie$general_position <- "GK"
  ...
  # Calculate percentage of season played (using 2304 minutes as full season)
  season.full <- season.full %>%
    dplyr::mutate(
      pct.season.played = minutes_played / 2304
    )

  # Fetch player details to get ages
  ids <- unique(season.full$player_id)
  players <- asa_client$get_players(leagues = "nwsl", ids = ids)
  ...
  nwsl.final <- stats::na.omit(nwsl.final)

  # Save to data-raw
  write.csv(nwsl.final, "data-raw/nwsl_final.csv", row.names = FALSE)
  ...
  if (!is.null(output_path)) {
    ...
    write.csv(nwsl.final, file.path(output_path, "nwsl_tidy.csv"), row.names = FALSE)
    message("NWSL data saved to ", file.path(output_path, "nwsl_tidy.csv"))
    return(invisible(file.path(output_path, "nwsl_tidy.csv")))
  } else {
    return(nwsl.final)
